...
int ensure_core_btf_with_tar_binary(const char **path, const char *tar_bin, int tar_len);
...
static int ensure_core_btf_tar(struct bpf_object_open_opts *opts, const char *tar_bin, int tar_len)
{
	return ensure_core_btf_with_tar_binary(&opts->btf_custom_path, tar_bin, tar_len);
}
